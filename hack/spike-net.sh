...
#   dns-vip    Docker's embedded DNS (127.0.0.11) forwards names it does not
...
#   host-vip   informational: the macOS host cannot reach the VIP, because
#              the bridge exists only inside the colima VM
#
# Usage:
#   hack/spike-net.sh        run all checks; removes leftover spike
#                            containers first and leaves klite0 behind
...
if docker network inspect "$NET" >/dev/null 2>&1; then
  ...
  if [ "$have" = "$SUBNET" ]; then
    ...
    record FAIL network "$NET exists with subnet $have and could not be replaced"
  fi
...
# ndots:1 matters. Docker writes "options ndots:0" by default, and musl (the
# libc in Alpine) skips the search list whenever a name has at least ndots
# dots, so a bare "b" would be queried literally and never as b.svc.klite.
# With ndots:1, single-label names go through the search list first.
...
if printf '%s' "$out" | grep -q "$VIP"; then
  ...
  record FAIL dns-vip "nslookup $FQDN did not return $VIP"
  printf '%s\n' "$out" | sed 's/^/  [nslookup] /'
fi

# http-name: bare name + search domain + VIP, end to end.
...
# bonus, DNS alone is enough (colima guests sometimes lack HTTP egress).
if docker exec "$CLIENT" wget -qO- -T 5 http://example.com >/dev/null 2>&1; then
  ...
  record PASS upstream "example.com resolves; HTTP fetch failed (egress blocked?)"
else
  record FAIL upstream "example.com does not resolve from $CLIENT"
  docker exec "$CLIENT" nslookup example.com 2>&1 | sed 's/^/  [nslookup] /'
fi

# host-vip: informational only. The design assumes container addresses are
# not reachable from macOS; record whatever this machine actually does.
hostbody=$(curl -m 2 -s "http://$VIP:8080" 2>/dev/null)
rc=$?
if [ "$rc" -eq 0 ] && [ "$hostbody" = "$BODY" ]; then
  record INFO host-vip "UNEXPECTED: macOS host CAN reach $VIP:8080 (host/bridge boundary does not hold here)"
else
  record INFO host-vip "macOS host cannot reach $VIP:8080 (curl exit $rc), as the design assumes"
fi
