...
test('killing an instance brings it back with a restart tally', async ({ page }) => {
  ...
  // the displayed phase dwells a second per beat: the real backoff is 500ms,
  // so Failed still showing after 700ms proves the dwell holds
  await expect(chip).toHaveAttribute('data-phase', 'Failed', { timeout: 5_000 })
  await page.waitForTimeout(700)
  await expect(chip).toHaveAttribute('data-phase', 'Failed')
  // then the durable outcome
...
test('live flow: flights run fast and untraced, the panel keeps the latest call', async ({ page }) => {
  // the flow hook previews live pacing on mock data: the header toggle
  // swaps the data source itself, and no facade runs in this suite
  await page.goto('/')
  await page.evaluate(() => {
    ;(window as { __kliteFlow?: (f: string) => void }).__kliteFlow?.('live')
  })
  await fourX(page)
