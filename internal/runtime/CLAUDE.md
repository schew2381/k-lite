...
This package holds the Runtime interface and its Docker implementation. It's the blast shield from ADR 0019: the moby SDK is v0.x and breaks between minors, so this package is the *only* importer, and everything else sees our interface.
...
- Netns-scoped options (add-host, NET_ADMIN, static IP, published ports) go on the donor container. Docker rejects them on a container joining another's network. The M9 ingress slice publishes host ports, so it rides the donor like everything else.
- Infra containers also carry `io.klite.cluster` (M8). Eviction keys on it: another cluster's containers are never removed, whatever they squat on.
