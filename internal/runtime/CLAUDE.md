...
- Netns-scoped options (add-host, NET_ADMIN, static IP, published ports) go on the donor container. Docker rejects them on a container joining another's network. The M9 ingress slice is published ports, so it rides the donor like everything else.
