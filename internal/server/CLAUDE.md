...
This package is klited's gRPC surface, serving ClusterService for clients, AgentService for nodes, ADS for Envoys, and the command hub that bridges log streams. Every replica runs all of it, since statelessness is the point (ADR 0005).
...
- Every RPC passes auth.go's deny-by-default gate (ADR 0028): node certs open AgentService and xDS, the admin bearer opens ClusterService, Register alone is public. New services must claim a caller class there or stay unreachable. xDS streams are additionally bound to the certificate's node (M9, closing ADR 0028's residual): the first request must name it and a mid-stream rename dies.
- Per-node streams (WatchDesired, commands) resend a full snapshot after any watch error (compaction is normal, but dying on it is not).
