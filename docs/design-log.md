...
## The playground grows up

A "fresh" reseed for the open-internet test came up with three-hour-old records on a one-minute etcd. The AGE column's lie unwound into a state-lifecycle hole: etcd bind-mounts, the CA, and node identities all outlived `dev-down --all`. Every reset since M1 had been resuming the old cluster rather than starting one, and demo.sh had quietly carried its own wipe as a workaround. ADR 0045 gives `--all` its honest meaning (the cluster's state goes with it) while a plain down still resumes.

The first real remote node then paid for itself twice over. The laptop joined over the tailnet from a phone hotspot and its instances ran. But every call it sent toward a Mac-hosted endpoint died, because open mode still advertised the Wi-Fi address the hotspot can't route. The failure pattern in the user's own tail (policy denials correct, local picks fine, remote picks dead) diagnosed it, and ADR 0049 moved the advertise onto the overlay. The same tail session produced ADR 0050: send lines now say `send ->` and verbose httpd logs receives. Naming the denying policy on failures was weighed and skipped, because a TCP reset carries no reason and the reach checker already owns that answer.

The same pass folded in two user asks. The board stopped being hand-started: dev-up and demo now build and run one loopback facade serving the built dist, with Vite retired from scripted flows (ADR 0046, leaning on the frontend's ADR 0040 trust boundary). And v0.1.1 shipped the pieces the internet story needed, the kdns attribution ring (0044) and the tailscale-mode join.sh (0043), after a release-pipeline lesson. A frontend type slip in the non-essential ui job painted the whole tag red, so that job is best-effort now. The tarball was rebuilt from the fix and attached by hand.
