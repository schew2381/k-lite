...
A "fresh" reseed for the open-internet test came up with three-hour-old records on a one-minute etcd, and the AGE column's lie unwound into a state-lifecycle hole: etcd bind-mounts, the CA, and node identities all outlived `dev-down --all`, so every reset since M1 had been resuming the old cluster rather than starting one. demo.sh had quietly carried its own wipe as a workaround. ADR 0045 gives `--all` its honest meaning — the cluster's state goes with it — while a plain down still resumes.

The same pass folded in two user asks. The board stopped being hand-started: dev-up and demo now build and run one loopback facade serving the built dist, Vite retired from scripted flows (ADR 0046, leaning on the frontend's ADR 0040 trust boundary). And v0.1.1 shipped the pieces the internet story needed — the kdns attribution ring (0044) and the tailscale-mode join.sh (0043) — after a release-pipeline lesson: a frontend type slip in the non-essential ui job painted the whole tag red, so that job is best-effort now, and the tarball was rebuilt from the fix and attached by hand.
