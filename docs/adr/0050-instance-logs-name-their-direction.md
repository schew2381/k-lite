...
- The serve line carries a literal backslash (`${l#*\[}`), and a single-quoted JavaScript string silently eats an unknown escape like `\[`. The generator doubles it to `\\[` to emit the right byte, and any future port of these lines into JS or TS must do the same, or the pattern ships broken while every parity eyeball says it matches.
