...
- `GET /api/traffic` (ADR 0024) is still unserved, so live flow shows no dots against today's facade. The flow machinery is ready for the feed rather than blocked on it, and `?flow=live` previews the behavior on the mock.
