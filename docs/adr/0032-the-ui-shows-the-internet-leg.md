...
3. **Build on M9's own field names, with the port as a sim-only enrichment** (chosen). `Node.status.advertiseAddress` is real and watchable, so both modes read it. The ingress port rides NetDesired live, so the sim stamps it on instance status for the demo and the UI degrades to the address alone when it's absent.
...
- When M9 exposes ports to clients (a facade enrichment or a status field), the sim-only comment on `ingressPort` comes off and nothing else moves.
