...
- Internet joins stay a copied command, now built on the browser's hostname, or on the facade machine's own interface addresses when the browser only knows localhost. The command also carries an advertise-address placeholder, because a remote node left to guess advertises its Docker bridge and other nodes dial into nothing. No facade route can reach a machine it doesn't run on.
